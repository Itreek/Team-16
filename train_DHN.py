...
def main(args):
    ...
    # model
    model = DHN(element_dim, hidden_dim, target_size,bidirectional, minibatch, is_train=True)
    model = model.train()
        ...
        for X, target in train_dataloader:
            ...
            if iteration % args.print_test == 0:
                ...
                for test_num, (data, target) in enumerate(val_dataloader):
                    ...
                    weight = torch.cat([wn_val, 1.0 - wn_val], dim=1)
                    # print weight
                    weight = weight.view(-1, 2, 1, 1).contiguous()
                    # print weight

                    loss = 10.0*focal_loss(new_weights, target.float(), weights=weight)

                    test_loss.append(float(loss.item()))

                    predicted, curr_acc = eval_acc(new_weights.float().detach(), target.float().detach(), weight.detach())
                    acc.append(curr_acc)
                ...
                print('Epoch: [{}][{}/{}]\tLoss {:.4f}\tweighted Accuracy {:.2f} %'.format(epoch, iteration % len(train_dataloader.dataset),len(train_dataloader.dataset),np.mean(np.array(test_loss)),100.0*np.mean(np.array(acc))))
                is_best = False
            iteration += 1
